...
export default function InteractiveTable() {
  ...
  const renderDishIcon = (dishId: string, className = "w-6 h-6 text-[#B8826A]") => {
    switch (dishId) {
      ...
      case 'it3': return <Cake className={className} />;
      case 'it4': return <Wine className={className} />;
      case 'it5': return <Award className={className} />;
    ...
    <section id="mesa-interactiva" className="relative py-20 bg-background overflow-hidden border-b border-primary/30">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="text-center max-w-3xl mx-auto mb-12">
          <span className="font-cursive text-4xl text-accent block mb-2 leading-none">El arte de la hospitalidad</span>
          ...
          <p className="text-sm sm:text-base text-brown/90 leading-relaxed font-sans max-w-2xl mx-auto">
            ...
            Haz clic o desliza con el dedo sobre los platos de nuestro mantel artesanal.
          </p>
        </div>
        ...
        <div className="max-w-4xl mx-auto">
          ...
          >
            <div className="absolute inset-y-0 left-2.5 flex items-center justify-center z-20 sm:hidden pointer-events-none">
              <button onClick={handlePrev} className="w-8 h-8 rounded-full bg-black/25 text-white flex items-center justify-center pointer-events-auto active:bg-accent/80 transition-colors shadow-md">
            ...
            <div className="absolute inset-y-0 right-2.5 flex items-center justify-center z-20 sm:hidden pointer-events-none">
              <button onClick={handleNext} className="w-8 h-8 rounded-full bg-black/25 text-white flex items-center justify-center pointer-events-auto active:bg-accent/80 transition-colors shadow-md">
                <span className="text-lg font-bold leading-none select-none">›</span>
              </button>
            </div>
            ...
            <div className="absolute inset-6 sm:inset-12 z-10 grid grid-cols-5 h-full items-center justify-center">
              {interactiveDishes.map((dish, idx) => {
                ...
                const positions = [
                  "col-start-1 row-start-2 -translate-y-8 sm:-translate-y-12",
                  "col-start-2 row-start-1 translate-y-4 sm:translate-y-2",
                  "col-start-3 row-start-2 -translate-y-8 sm:-translate-y-12",
                  "col-start-4 row-start-3 -translate-y-2 sm:-translate-y-4",
                  "col-start-5 row-start-2 -translate-y-8 sm:-translate-y-12",
                ];

                return (
                  <div key={dish.id} className={`flex flex-col items-center justify-center relative ${positions[idx] || ""}`}>
                    <motion.div
                    ...
                    >
                      ...
                      <div className="mt-2 text-center flex flex-col items-center">
                        <span className={`text-[10px] sm:text-xs font-serif tracking-wide transition-all duration-300 ${
                          isSelected ? 'text-white font-bold scale-105' : 'text-white/85 font-medium group-hover:text-white'
                        }`}>
                          {dish.name}
                        </span>
                        <motion.div 
